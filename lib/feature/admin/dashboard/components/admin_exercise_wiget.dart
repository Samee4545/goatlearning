...
import 'package:goatlearning/feature/admin/dashboard/view/admin_exercise_detail_screen.dart';
import 'package:goatlearning/feature/admin/edit/controller/admin_edit_controller.dart';
import 'package:shimmer/shimmer.dart';

class AdminExerciseWidget extends StatelessWidget {
  AdminExerciseWidget({super.key});
            ...
            Expanded(
              child: Obx(() {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final item = adminEditController.filteredContent[index];

                    // Folder tile with expandable chapters
                    if (item.isFolder) {
                      return Obx(() {
                        final isExpanded = adminEditController
                    ...
                    return Container(
                      ...
                      child: Material(
                        ...
                        child: InkWell(
                          borderRadius: BorderRadius.circular(12.0),
                          onTap: () async {
                            final chapterId = item.chapterId ?? item.id ?? '';
                            if (chapterId.isEmpty) return;
                            await adminEditController.getChapterById(chapterId);
                            final ch = adminEditController.selectedChapter;
                            if (ch != null) {
                              Get.to(
                                () => AdminExerciseDetailScreen(chapter: ch),
                              );
                            }
                          },
                          child: Padding(
