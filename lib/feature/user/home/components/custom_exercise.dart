                ...
                Expanded(
                  child: CustomScrollView(
                    slivers: [
                      ...
                      // ── Premium upsell (shown only when locked) ────────
                      if (!isFullyUnlocked)
                        SliverToBoxAdapter(
...
class _FolderTile extends StatelessWidget {
  const _FolderTile({required this.item, required this.controller});
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final isExpanded =
      controller.expandedFoldersExercise.contains(item.folderId);
        ...
        children: [
          ...
          Container(
            ...
            child: Material(
              ...
              child: InkWell(
                borderRadius: BorderRadius.circular(12),
                onTap: () => controller.toggleFolderExpansionExercise(
                  item.folderId ?? '',
                ),
                child: Padding(
                  padding: const EdgeInsets.all(16),
          ...
          AnimatedSize(
            ...
            child: (isExpanded && folderChapters != null)
              ...
              children: folderChapters.asMap().entries.map((entry) {
                ...
                final exerciseCount = chapter['exerciseCount'] ?? 0;
...
class _FolderChapterTile extends StatelessWidget {
  const _FolderChapterTile({
              ...
              children: [
                ...
                Obx(() {
                  ...
                  if (contentItem.isNotEmpty) {
                    ...
                    isFav = controller.guestFavouriteIds.contains(chapterId);
                  }
...
class _ChapterTile extends StatelessWidget {
  const _ChapterTile({required this.item, required this.controller});
...
class _PremiumUpsell extends StatelessWidget {
  const _PremiumUpsell({required this.profileController});
